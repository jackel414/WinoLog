...
task :run do
  pids = [
    spawn("cd backend && EMBER_PORT=4200 rails s -p 3900"),
    spawn("cd frontend && ./node_modules/.bin/ember server --port=4200 --proxy=http://0.0.0.0:3900"),
  ]
...
task :test do
  pids = [
    spawn("cd backend && EMBER_PORT=4200 rails s -p 3900 -e test"),
